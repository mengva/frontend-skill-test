...
function ProductCartItem({ handlerShowDialogRemove }: ProductCartItemDto) {
    ...
    return (
        <>
            <div style={{ userSelect: 'none' }} className='show-btn-remove cursor-pointer relative grid grid-cols-3 mb-2 bg-slate-100 hover:bg-slate-200 px-4 py-3 rounded-xl'>
                ...
                <div>
                    ...
                    <p className='mt-1 text-slate-600'>Type: <span className='text-[#cf3bed]'>{"Shoes"}</span></p>
                </div>
                ...
                <div className='flex justify-end items-center'>
                    <p className='mt-1 text-slate-600'>Total: <span className='text-sky-500'>${totalPice}</span></p>
                </div>
            </div>
        </>
    )
