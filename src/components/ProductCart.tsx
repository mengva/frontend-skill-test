    ...
    return (
        <>
            <div ref={container} onClick={onCloseModel} className={`fixed overflow-hidden w-full h-full z-10 top-0 left-0 bg-slate-500/[.5] ${isTranslate ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <div className={`sm:w-[450px] w-full absolute top-0 right-0 bg-slate-50 cart-scrolling h-full ${isTranslate ? 'translate-x-0' : 'translate-x-full'}`}>
                    <div className="w-full h-full pb-[80px] overflow-y-auto absolute inset-0">
                        <div className='flex justify-start items-center w-full h-[60px] z-[2] px-1 sticky top-0 bg-slate-50'>
                    ...
                    <div className="absolute bottom-0 right-0 w-full h-[80px] px-5 flex justify-between items-center bg-slate-200">
                        <div className="flex items-center h-full w-full">
                            <div className="text-xl text-slate-700 font-semibold">Total Price: <span className="text-[#cf3bed]">$123.123</span></div>
                        </div>
